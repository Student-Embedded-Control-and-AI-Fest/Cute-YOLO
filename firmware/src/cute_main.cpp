...
void setup()
{
    ...
    Serial.println(
        "SERIALDATA READY -- collector should send RDYSAMPLE heartbeat.");
    NoodleSerial::print_ready();
}
...
void loop()
{
    ...
    // Optional PC dataset collector heartbeat. This is non-blocking.
    poll_serial_dataset_ready();

    // Activate a verified BLE upload only from the main loop, never from the
    // BLE callback, so no inference worker can be using the old mmap.
    cute_model_poll();

    if (cute_model_uploading())
    {
        status("BLE UPDATE", ST77XX_YELLOW);
